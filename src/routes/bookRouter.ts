...
import {
    bookByUserIdQueryValidation,
    bookGenreQueryValidation,
    bookRemoveQueryValidation
} from "../joiSchemas/bookSchema.js";
...
bookRouter.get('/booksByUser', async (req: Request, res: Response) => {
    const {error} = bookByUserIdQueryValidation.validate(req.query);
    if (error) throw new HttpError(400, error.message)
